...
const NotificationSystem = {
    ...
    addNotification: function (appt, type = 'callback_due') {
        ...
        const existing = this.notifications.find(n => n.appointmentId === appt.id &&
        ...
        const recentDuplicate = this.notifications.find(n => n.appointmentId === appt.id &&
            n.type === type &&
            !n.dismissed &&
    ...
    showCallbackModal: function (appt) {
        if (!appt || document.getElementById('callbackDueModal'))
            return;
        ...
        overlay.querySelector('.callback-finish-btn')?.addEventListener('click', () => { if (typeof Data !== 'undefined' && Data.finishCallback)
            Data.finishCallback(appt.id); close(); });
    ...
    processPopupQueue: function () {
        if (this.isPopupShowing || this.popupQueue.length === 0)
            return;
    ...
    attachPopupEvents: function (popup, popupId, notification) {
        ...
        popup.querySelector('.finish-callback-btn')?.addEventListener('click', () => {
            if (typeof Data !== 'undefined' && Data.finishCallback)
                Data.finishCallback(notification.appointmentId);
            this.dismissPopup(popupId, notification.id);
        });
    ...
    removeCallbackNotification: function (appointmentId) {
        ...
        popupIds.forEach(pid => { const el = document.getElementById(pid); if (el?.dataset?.appointmentId === String(appointmentId))
            el.remove(); });
        this.saveNotifications();
        this.updateBadge();
        this.renderDropdown();
    ...
    attachDropdownEvents: function (body) {
        ...
        body.querySelectorAll('.finish-callback-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                if (typeof Data !== 'undefined' && Data.finishCallback)
                    Data.finishCallback(btn.dataset.apptId);
            });
        });
    ...
    cleanupExpired: function () {
        ...
        this.notifications = this.notifications.filter(n => {
            ...
            if ((n.type === 'callback_due' || n.type === 'callback_overdue') && !n.dismissed)
                return true;
