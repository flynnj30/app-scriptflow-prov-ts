...
(function () {
    ...
    function quietPuterSdkLogs() {
        if (window.__scriptflowPuterConsoleGuard)
            return;
        const original = { log: console.log, info: console.info, warn: console.warn, error: console.error };
        const isKnownPuterNoise = (args) => {
            const text = args.map(v => { try {
    ...
    const Studio = {
        ...
        init() {
            ...
            document.addEventListener('click', (e) => {
                ...
                if (item && typeof FeaturePanel !== 'undefined')
      ...
      <div class="ts-pro">
        ...
        bindUpload(container) {
            ...
            zone.onclick = (e) => { if (e.target.closest('button'))
                return; input.click(); };
            ...
            zone.addEventListener('drop', e => { const f = e.dataTransfer.files && e.dataTransfer.files[0]; if (f)
                this.handleSource(f, container); });
            ...
            container.querySelector('#tsUrlInput').addEventListener('keydown', e => { if (e.key === 'Enter')
                this.loadUrl(container); });
        },
        async loadUrl(container) {
            const url = container.querySelector('#tsUrlInput').value.trim();
            if (!/^https?:\/\//i.test(url))
                return this.uploadError(container, 'Enter a valid http(s) audio or video URL.');
        ...
        handleSource(file, container) {
            ...
            if (!isText && !isAudio && !isVideo)
                return this.uploadError(container, 'Unsupported file type. Please upload an audio/video file or a transcript file.');
      ...
      <div class="ts-pro ts-config-page">
        ...
        bindConfigure(container) {
            ...
            audio.onloadedmetadata = () => {
                ...
                if (meta)
                    meta.innerHTML = `<i class="far fa-clock"></i> ${this.formatDuration(audio.duration)}`;
            };
        ...
        async transcribe(container) {
            if (this.state.busy || !this.state.file)
                return;
        ...
        async ensurePuterReady() {
            ...
            while (!ready()) {
                if (Date.now() - started > 5000)
                    throw new Error('Puter AI loaded but speech-to-text is not available.');
        ...
        async ensurePuterAuthorized() {
            ...
            try {
                if (sdk.auth && typeof sdk.auth.isSignedIn === 'function' && sdk.auth.isSignedIn())
                    return true;
        ...
        chunksFromText(text, duration) {
            ...
            if (!sentences.length)
                return [{ start: 0, end: Number(duration) || 0, text }];
        ...
        friendlyPuterError(error) {
            ...
            if (/sign.?in|auth|permission|puter account/i.test(message))
                return 'Puter AI needs authorization to transcribe this recording. Please sign in to Puter when prompted and try again.';
            if (/file|blob|size|payload|too large/i.test(message))
                return 'The audio file could not be submitted. Try a shorter recording or a smaller compressed file.';
            if (/unsupported|format|codec|decode/i.test(message))
                return 'This audio format is not supported by the transcription service. Try MP3, WAV, M4A, OGG/OPUS, or FLAC.';
            if (/network|fetch|timeout|connection|gateway|service|websocket|socket|closed before/i.test(message))
                return 'Puter AI could not establish a stable connection. Please try again; the app will automatically retry transient connection failures.';
            return message;
        },
        // ------------------------------------------------------------
        ...
        extractBookingData(text) {
            ...
            const firstMatch = (patterns) => {
                for (const pattern of patterns) {
                    ...
                    if (match && match[1] && cleanValue(match[1]))
        ...
        extractSchedule(text) {
            ...
            if (!raw)
                return { raw: '', display: '', date: '', time: '', timezone: '' };
        ...
        parseFlexibleDate(value) {
            ...
            if (m) {
                ...
                if (d.getFullYear() === Number(year) && d.getMonth() === mi && d.getDate() === Number(m[2]))
                    return `${d.getFullYear()}-${String(mi + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
            }
            m = s.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);
            if (m)
                return this.validDateParts(Number(m[1]), Number(m[2]) - 1, Number(m[3]));
            m = s.match(/^(\d{1,2})[\/\\-](\d{1,2})[\/\\-](\d{2,4})$/);
            if (m) {
                const y = Number(m[3]) < 100 ? 2000 + Number(m[3]) : Number(m[3]);
                return this.validDateParts(y, Number(m[1]) - 1, Number(m[2]));
            }
            return '';
        },
        ...
        ordinal(day) { const n = Number(day); if (n % 100 >= 11 && n % 100 <= 13)
            return 'th'; return ({ 1: 'st', 2: 'nd', 3: 'rd' }[n % 10] || 'th'); },
        detectInterest(text) {
            const value = String(text || '');
            if (/(?:high interest|very interested|highly interested|excited|enthusiastic|love (?:it|that)|absolutely|definitely|looking forward|can't wait|strong interest)/i.test(value))
                return 'High';
            if (/(?:medium[- ]high|interested|considering|curious|open to|sounds good|that works|okay|sure|willing to|would like|happy to|fine with)/i.test(value))
                return 'Medium';
            if (/(?:low interest|not interested|not sure|maybe later|too busy|already have|don't need|not looking|skeptical|reserved|just looking|hesitant)/i.test(value))
                return 'Low';
        ...
        buildMeetingNotes(text, meta) {
            ...
            if (interest === 'High')
                meetingAngle = 'Keep the meeting focused, lead with the prospect’s primary goal, and show the strongest relevant features first.';
            if (interest === 'Medium')
                meetingAngle = 'Use the walkthrough to compare the preview against the prospect’s current setup and priorities.';
            if (interest === 'Low')
                meetingAngle = 'Use discovery first, show only the strongest improvements, and avoid pushing for a decision.';
            return [
                `Attendees: ${attendee}.`,
      ...
      <div class="ts-pro ts-result-page">
        ...
        renderChunks() {
            if (!this.state.chunks.length)
                return `<div class="ts-text-only">${this.esc(this.state.transcript).replace(/\n/g, '<br>')}</div>`;
        ...
        audioControls(duration) {
            if (this.state.sourceType !== 'audio')
                return '<div class="ts-no-audio"><i class="fas fa-file-lines"></i> Imported transcript — audio controls are unavailable.</div>';
        ...
        bindResult(container) {
            ...
            if (audio) {
                audio.onloadedmetadata = () => { this.state.audioDuration = audio.duration || this.state.audioDuration; const seek = container.querySelector('#tsSeek'); if (seek)
                    seek.max = audio.duration || 1; };
                audio.ontimeupdate = () => { const seek = container.querySelector('#tsSeek'); const t = container.querySelector('#tsCurrentTime'); if (seek)
                    seek.value = audio.currentTime; if (t)
                    t.textContent = this.formatClock(audio.currentTime); this.highlightChunk(container, audio.currentTime); };
                audio.onended = () => { if (play)
                    play.innerHTML = '<i class="fas fa-play"></i>'; };
            }
            if (play)
                play.onclick = () => { if (audio.paused) {
                    audio.play();
            ...
            const seek = container.querySelector('#tsSeek');
            if (seek)
                seek.oninput = () => { audio.currentTime = Number(seek.value); };
            const rewind = container.querySelector('#tsRewind');
            if (rewind)
                rewind.onclick = () => { audio.currentTime = Math.max(0, audio.currentTime - 10); };
            const speed = container.querySelector('#tsSpeed');
            if (speed)
                speed.onchange = () => { audio.playbackRate = Number(speed.value.replace('x', '')); };
            const vol = container.querySelector('#tsVolume');
            if (vol)
                vol.onclick = () => { audio.muted = !audio.muted; vol.innerHTML = `<i class="fas ${audio.muted ? 'fa-volume-xmark' : 'fa-volume-high'}"></i>`; };
            container.querySelectorAll('.ts-chunk').forEach(el => el.onclick = () => { this.state.selectedChunkIndex = Number(el.dataset.index || 0); audio.currentTime = Number(el.dataset.start || 0); audio.play(); if (play)
                play.innerHTML = '<i class="fas fa-pause"></i>'; });
            const search = container.querySelector('#tsSearch');
            if (search)
                search.oninput = () => this.filterChunks(container, search.value);
            container.querySelector('#tsCopy').onclick = () => this.copyText(this.state.transcript);
            const bookingCopy = container.querySelector('#tsCopyBooking');
            if (bookingCopy)
                bookingCopy.onclick = () => this.copyText(container.querySelector('#tsBookingText')?.value || '');
            const bookingSend = container.querySelector('#tsSendBooking');
            if (bookingSend)
                bookingSend.onclick = () => this.sendBookingToSmartImport(container);
            ...
            const add = container.querySelector('#tsAddSpeaker');
            if (add)
                add.onclick = () => this.addSpeakerLabel(container);
            container.querySelectorAll('[data-analysis]').forEach(tab => tab.onclick = () => this.renderAnalysisTab(container, tab.dataset.analysis));
            const exportMenu = container.querySelector('#tsExportMenu');
            if (exportMenu)
                exportMenu.onclick = () => { container.querySelector('#tsExportDrawer').hidden = false; };
            const closeExport = container.querySelector('#tsCloseExport');
            if (closeExport)
                closeExport.onclick = () => { container.querySelector('#tsExportDrawer').hidden = true; };
            ...
            const regen = container.querySelector('#tsRegenerate');
            if (regen)
                regen.onclick = () => this.renderAnalysisTab(container, 'summary');
            const mapExport = container.querySelector('#tsExportMap');
            if (mapExport)
                mapExport.onclick = () => this.exportMindMap();
        },
        sendBookingToSmartImport(container) {
            ...
            if (typeof openSmartImportEnhanced === 'function') {
                ...
                if (typeof showToast === 'function')
                    showToast('Transcript booking details loaded into Smart Import. Review before saving.', 'success');
                return;
            }
            this.copyText(text);
            if (typeof showToast === 'function')
                showToast('Booking format copied. Paste it into Smart Import.', 'info');
        },
        renderAnalysisTab(container, tab) {
            ...
            if (tab === 'summary')
                body.innerHTML = this.summaryPanel(this.makeSummary(this.state.transcript, this.state.summaryMode === 'detailed' ? 'detailed' : 'concise'));
            if (tab === 'mindmap')
                body.innerHTML = this.mindmapPanel(this.state.transcript);
            if (tab === 'insights')
                body.innerHTML = this.insightsPanel(this.state.transcript);
        },
        summaryPanel(s) {
        ...
        mindmapPanel(text) {
            const topics = this.topTopics(text, 7);
        ...
        insightsPanel(text) {
            const s = this.makeInsights(text);
        ...
        makeSummary(text, mode) {
            const sentences = this.sentences(text);
            if (!sentences.length)
                return { overview: 'No transcript available.', points: [], takeaways: [] };
            const limit = mode === 'detailed' ? 8 : 4;
            const points = sentences.slice(0, Math.min(limit, sentences.length));
        ...
        topTopics(text, count) {
            ...
            const words = (text.toLowerCase().match(/[a-z][a-z'-]{4,}/g) || []).filter(w => !stop.has(w));
            const freq = {};
        ...
        highlightChunk(container, time) { let best = -1; this.state.chunks.forEach((c, i) => { if (time >= c.start && (c.end === 0 || time <= c.end))
            best = i; }); container.querySelectorAll('.ts-chunk').forEach((el, i) => el.classList.toggle('playing', i === best)); },
        addSpeakerLabel(container) {
            ...
            if (scroll)
                scroll.innerHTML = this.renderChunks();
            this.bindResult(container);
        },
        async quickTranslate(container) {
            if (!this.state.transcript)
                return;
            if (this.state.sourceType !== 'audio' || !this.state.file) {
                if (typeof showToast === 'function')
                    showToast('Translation requires the original audio file.', 'info');
                return;
            }
            ...
            try {
                ...
                this.bindResult(container);
                if (typeof showToast === 'function')
                    showToast('English translation generated.', 'success');
            }
            catch (err) {
                if (typeof showToast === 'function')
                    showToast(this.friendlyPuterError(err), 'error');
            }
            finally {
        ...
        exportMindMap() {
            ...
            a.download = `${this.slug(this.baseName(this.state.fileName)) || 'transcript'}-mind-map.txt`;
        ...
        async shareTranscript() {
            ...
            if (navigator.share) {
                try {
                    await navigator.share({ title: this.baseName(this.state.fileName), text });
                    return;
                }
                catch (_) { }
            }
        ...
        moreMenu(container) {
            const drawer = container.querySelector('#tsExportDrawer');
            if (drawer) {
                drawer.hidden = false;
            }
        },
        export(text, type) {
            ...
            if (type === 'doc') {
                data = `<!doctype html><html><body><h1>${this.esc(this.baseName(this.state.fileName))}</h1><p>${this.esc(this.lastSummary || '')}</p><pre>${this.esc(text)}</pre></body></html>`;
                mime = 'application/msword';
            }
            if (type === 'pdf') {
                this.print(text, this.lastSummary);
                return;
            }
            const blob = new Blob([data], { type: mime });
            const a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            a.download = `${this.slug(this.baseName(this.state.fileName)) || 'scriptflow-transcript'}.${ext}`;
            document.body.appendChild(a);
            a.click();
            a.remove();
            setTimeout(() => URL.revokeObjectURL(a.href), 1000);
        ...
        uploadError(c, msg) { const e = c && c.querySelector('#tsUploadError'); if (!e)
            return; e.hidden = !msg; e.textContent = msg || ''; },
        friendlyError(e) { const m = String(e && e.message || e || 'Unknown error'); if (/decode|audio|unsupported|AudioContext/i.test(m))
            return 'Your browser could not decode this audio format. Try OGG/Opus, WAV, MP3, M4A, or convert the file to WAV and retry.'; if (/memory|out of memory/i.test(m))
            return 'This recording is too large for the current browser memory. Try the Fast model or split the recording into shorter files.'; if (/network|fetch|load|cdn|model/i.test(m))
            return 'The AI model could not be loaded. Check your internet connection and try again.'; return m; },
        ...
        formatBytes(n) { if (!n)
            return '0 B'; const u = ['B', 'KB', 'MB', 'GB']; const i = Math.min(Math.floor(Math.log(n) / Math.log(1024)), u.length - 1); return `${(n / Math.pow(1024, i)).toFixed(i ? 1 : 0)} ${u[i]}`; },
        formatDuration(sec) { if (!isFinite(sec) || sec < 0)
            return '00:00'; const h = Math.floor(sec / 3600), m = Math.floor((sec % 3600) / 60), s = Math.floor(sec % 60); return h ? `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}` : `${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}`; },
        ...
        extFromUrl(u) { try {
            const p = new URL(u).pathname.match(/\.[a-z0-9]{2,5}$/i);
            return p ? p[0] : '';
        }
        catch (_) {
            return '';
        ...
        revokeUrl() { if (this.state.audioUrl && this.state.audioUrl.startsWith('blob:'))
            URL.revokeObjectURL(this.state.audioUrl); this.state.audioUrl = ''; },
        esc(s) { const d = document.createElement('div'); d.textContent = String(s ?? ''); return d.innerHTML; }
    };
    // Direct launcher used by the existing ScriptFlow buttons. The full Studio
    // implementation remains available for backward compatibility.
    const TRANSCRIPT_STUDIO_URL = 'https://transcript-studio-n0nv.onrender.com/';
    function redirectToTranscriptStudio() { window.location.assign(TRANSCRIPT_STUDIO_URL); }
    function bindTranscriptLauncher(id) {
        const el = document.getElementById(id);
        if (!el || el.dataset.transcriptStudioBound === 'true')
            return;
        el.dataset.transcriptStudioBound = 'true';
        el.addEventListener('click', redirectToTranscriptStudio);
        el.addEventListener('keydown', (event) => {
            if (event.key === 'Enter' || event.key === ' ') {
                event.preventDefault();
                redirectToTranscriptStudio();
            }
        });
    }
    function bindTranscriptLaunchers() {
        bindTranscriptLauncher('openTranscriptStudioBtn');
        bindTranscriptLauncher('openTranscriptStudioToolBtn');
    }
    window.TranscriptStudio = Studio;
    document.addEventListener('DOMContentLoaded', () => { Studio.init(); bindTranscriptLaunchers(); });
})();
