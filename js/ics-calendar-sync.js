...
(function (window) {
    ...
    const ICSCalendarSync = {
        ...
        mapTimezone(value) {
            ...
            if (v.includes('EASTERN') || v.includes('NEW_YORK') || v === 'EST' || v === 'EDT')
                return 'Eastern EDT';
            if (v.includes('MOUNTAIN') || v.includes('DENVER') || v === 'MST' || v === 'MDT')
                return 'Mountain MDT';
            if (v.includes('PACIFIC') || v.includes('LOS_ANGELES') || v === 'PST' || v === 'PDT')
                return 'Pacific PDT';
            if (v === 'UTC' || v === 'GMT')
                return 'UTC';
            return 'Central CDT';
        ...
        parse(text) {
            ...
            if (!/BEGIN:VCALENDAR/i.test(raw))
                throw new Error('Invalid ICS file: VCALENDAR header not found.');
        ...
        findExisting(event) {
            return this.existingAppointments().find(appt => {
                if (event.icsUid && appt.icsUid && String(appt.icsUid) === String(event.icsUid))
                    return true;
                return appt.source === 'ics' && appt.business === event.business && appt.date === event.date && appt.time === event.time;
            }) || null;
        },
        async importText(text) {
            if (typeof Data === 'undefined' || typeof AppState === 'undefined')
                throw new Error('Calendar is not ready yet.');
            const parsed = this.parse(text);
            if (!parsed.events.length)
                return { imported: 0, updated: 0, skipped: 0, total: parsed.totalEvents };
